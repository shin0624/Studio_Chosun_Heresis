...
//--------------------------에너미 행동 관련 변수-----------------------------------------
    ...
    [SerializeField] private const float ChaseRange = 8.0f;//플레이어 추격 가능 범위
    [SerializeField] private const float DetectionRange = 7.0f;// 플레이어 탐지 거리
    [SerializeField] private const float AttackRange = 3.0f;// 공격 가능 범위
...
//--------------------------에너미 경로 계산 관련 변수------------------------------------
    ...
    [SerializeField] private AudioSource ado2;// 러닝 시 내는 소리
    ...
    private void UpdateAttack()// 공격 후 -> 플레이어와의 거리가 공격 가능 범위를 넘어간 상태 && 플레이어와의 거리가 아직 탐지 범위에 포함될 때 다시 플레이어를 공격해야 함.
    {
