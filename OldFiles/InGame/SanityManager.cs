/*
 * SanityManager.cs
 * 이 스크립트는 플레이어의 정신력과 회복약을 관리하며,
 * 정신력 수치가 감소하거나 회복될 때의 처리를 담당
 * 게임 오버 조건과 관련된 기능도 포함
 */
using TMPro;
using UnityEngine;
...
public class SanityManager : MonoBehaviour
{
    ...
    public void DecreaseSanity()
    {
        ...
        if (sanity > 0)
        {
            ...
            if (sanity <= 0 && !gameOverTriggered)
            {
                ...
                audioManager.PlaySoundLoop(heartbeat);              // 심장 소리 반복 재생
    ...
    private IEnumerator DelayedRestoreSanity()
    {
        ...
        if (gameOverTriggered && sanity > 0)
        {
            ...
            audioManager.StopSoundLoop();           // 심장 소리 정지
