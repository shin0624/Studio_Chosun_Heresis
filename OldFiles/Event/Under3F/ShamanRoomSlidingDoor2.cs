...
public class ShamanRoomSlidingDoor2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        OpenLeftPosition = new Vector3(ClosedLeftPosition.x - OpenValue, ClosedLeftPosition.y, ClosedLeftPosition.z );//x축으로 슬라이딩 된 후의 문 위치값을 저장
        OpenRightPosition = new Vector3(ClosedRightPosition.x , ClosedRightPosition.y, ClosedRightPosition.z );
    }
    ...
    private void Update()// 업데이트에서 플레이어 트리거 진입 여부와 E키 입력을 체크. OnTriggerEnter에서 E키 입력을 즉시 확인하려 했으나, GetKeyDown이 짧은 시간동안만 동작하여 입력 처리에 오류가 발생했기 때문에 업데이트에서 처리
    {
        ...
        if (isOpening)//문 열림 상태이면
        {
            ...
            if (Vector3.Distance(LeftDoor.transform.position, OpenRightPosition) < 0.01f && Vector3.Distance(RightDoor.transform.position, OpenLeftPosition) < 0.01f)// 목표 위치까지 도달했다면
            {
                ...
                AudioManager.Instance.StopSound(DoorSound02);
                AudioManager.Instance.StopSound(DoorSound01);
            }
        }
    }
